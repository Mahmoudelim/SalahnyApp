...
class painterCatViewModel : ViewModel(){
...
// Request location updates
    ...
    suspend fun getPainter(): List<Worker> {
        val response = apiService.getAllWorkers()
        Log.i("respon", response.toString())
        val plumberWorkers = response.values.filter { it.profession == "Painter" && it.isAvailable }
    ...
    suspend fun getSortedWorker(context: Context): List<Worker> {
        ...
        val sortedPlumbers = Painter.sortedBy { carpen ->
