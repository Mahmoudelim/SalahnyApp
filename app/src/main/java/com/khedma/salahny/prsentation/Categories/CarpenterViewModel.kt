...
class CarpenterViewModel :ViewModel(){
...
// Request location updates
    ...
    suspend fun getCarpenter(): List<Worker> {
        val response = apiService.getAllWorkers()
        Log.i("respon", response.toString())
        val plumberWorkers = response.values.filter { it.profession == "Carpenter" && it.isAvailable }
    ...
    suspend fun getSortedWorker(context: Context): List<Worker> {
        ...
        // Fetch plumbers
        val Carpenter = getCarpenter()

        // Sort plumbers based on distance from user's location
        val sortedPlumbers = Carpenter.sortedBy { carpen ->
