...
class ElectricianCatViewModel: ViewModel(){
...
// Request location updates
    ...
    suspend fun getElectrician(): List<Worker> {
        val response = apiService.getAllWorkers()
        Log.i("respon", response.toString())
        val plumberWorkers = response.values.filter { it.profession == "Electrician" && it.isAvailable }
    ...
    suspend fun getSortedWorker(context: Context): List<Worker> {
        ...
        // Fetch plumbers
        val Electrician = getElectrician()

        // Sort plumbers based on distance from user's location
        val sortedPlumbers = Electrician.sortedBy { carpen ->
            userLocation?.let { userLoc ->
                calculateDistance(userLoc.lat, userLoc.long, carpen.location.lat, carpen.location.long)
